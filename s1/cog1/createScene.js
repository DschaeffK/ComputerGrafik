...
function(exports, scenegraph, animation) {
	...
	function init() {
		...
		var shoulder = scenegraph.createNodeWithModel("shoulder", "sphere", {recursionDepth: 2, scale: 80});
		var upperArm = scenegraph.createNodeWithModel("upperArm", "cube", {scale: 50}, shoulder);
		upperArm.scale([3, .05, .05]);
		upperArm.translate([96, -248, 0]);
		upperArm.rotate([0, 0, -1.2]);

		var elbow = scenegraph.createNodeWithModel("elbow", "sphere", {recursionDepth: 2, scale: 60}, shoulder);
		elbow.translate([180, -464, 0]);

		var lowerArm = scenegraph.createNodeWithModel("lowerArm", "cube", {scale: 35}, elbow);
		lowerArm.scale([5, 0, 0]);
		lowerArm.translate([246, -4, 0]);

		var wrist = scenegraph.createNodeWithModel("wrist", "sphere", {recursionDepth: 2, scale: 50}, elbow);
